...
The bot template includes `/checkaccounts` to test all non-deleted accounts. It sends an immediate acknowledgement, checks for unusable accounts, and then shows inline buttons for soft delete, hard delete, or cancel. Both soft and hard delete paths require two button confirmations.
