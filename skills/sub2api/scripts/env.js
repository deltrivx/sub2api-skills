/**
 * Shared configuration loader for Sub2API scripts.
 *
...
const BASE_URL = process.env.SUB2API_BASE_URL;
const ACCESS_TOKEN = process.env.SUB2API_ACCESS_TOKEN;
const USER_ID = process.env.SUB2API_USER_ID || "";

if (!BASE_URL || !ACCESS_TOKEN) {
  const missing = [
    !BASE_URL && "SUB2API_BASE_URL",
    !ACCESS_TOKEN && "SUB2API_ACCESS_TOKEN",
  ].filter(Boolean);
  console.error(
    `[CONFIG_MISSING] ${missing.join(", ")}\n` +
    "The user has not configured the required environment variables.\n" +
    "DO NOT retry. Ask the user to export them (e.g. `export SUB2API_BASE_URL=...`) or set them in a .env file."
  );
  process.exit(2);
}
