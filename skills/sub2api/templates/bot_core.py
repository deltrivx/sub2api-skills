#!/usr/bin/env python3
"""
Sub2API Bot 公共命令处理模块（与传输层解耦）。

本模块包含所有与 Sub2API 后端交互、PostgreSQL 查询、账号导入、
确认保护、备份、更新、重启等业务命令函数。它不直接调用任何
IM 平台（Telegram / QQ）的 API，所有回复都通过返回字符串交给
调用方（telegram-bot.py / qq-bot.py）发送。

传输层（Telegram/QQ）只需：
  1. 接收消息文本 / 文件，调用 handle_command / handle_document
  2. 把返回的字符串回复给用户
  3. 实现 Backend 抽象（用于 /restart bot 自杀、回调按钮等可选能力）
...
# 允许的用户/会话白名单（逗号分隔）。Telegram 用 chat_id；QQ 用 user_openid / channel_id。
ALLOWED_CHAT_IDS = {x.strip() for x in os.environ.get("SUB2API_BOT_ALLOWED_CHAT_IDS", "<chat-id>").split(",") if x.strip()}
...
def load_secrets():
    ...
    return {
        ...
        "qq_app_id": os.environ.get("QQ_APP_ID", "<qq-app-id>"),
        "qq_app_secret": os.environ.get("QQ_APP_SECRET", "<qq-app-secret>"),
...
def cmd_status():
    ...
    if runtime:
        ...
        for rid in sorted(runtime, key=lambda x: int(x)):
            ...
            lines.append(f"- #{rid} {item['name']}: 状态:{item.get('status') or '未知'} | 路由:{yesno(item.get('sched'))}{suffix}")
    else:
...
def cmd_logs():
    sections = []
    sections.append("sub2api_bot.log：\n" + (tail_file("/tmp/sub2api_bot.log", 20) or "无"))
...
def restart_target_command(target):
    if target in ("bot", "telegram", "telegram-bot", "qq", "qqbot", "qq-bot"):
        return "重启 Bot 容器", "kill -TERM 1"
...
def cmd_restart(text, backend=None):
    """backend 是传输层对象，需实现 send_message(chat_id, text, buttons=None) 和 restart_buttons()。
    如果 backend 为 None（不支持按钮），将退化为文字提示。"""
    parts=text.split()
    if len(parts)<2:
        if backend is not None:
            backend.send_message(None, "请选择要重启的服务：Bot / Sub2API", backend.restart_buttons())
            return None
        return "请选择要重启的服务：Bot / Sub2API\n用法：/restart bot|sub2api"
...
def handle_document_payload(raw, file_name, mime_type=""):
    """处理原始字节文件（传输层无关）：传入 raw bytes、文件名、mime，
    返回应回复的字符串。Telegram/QQ 传输层各自负责下载文件得到 raw 后调用本函数。"""
    size = len(raw)
...
def cmd_update(backend=None):
    """检测更新。backend.send_message 可选，用于立即提示"正在检测"。
    返回最终应回复的字符串，或 None（已经通过 backend 发送过）。"""
    is_docker = docker_enabled() and pathlib.Path(docker_compose_dir()).exists()
    if backend is not None:
        if is_docker:
            backend.send_message(None, "正在检测 Docker 版 Sub2API 镜像更新，请稍等…")
        else:
            backend.send_message(None, "正在检测系统级 Sub2API 更新，请稍等…")
    if is_docker:
        image = docker_sub2api_image()
        try:
        ...
        return make_confirm("update", "更新 Sub2API Docker 容器。\n" + detail, docker_update_command())
...
def handle_command(text, chat_id="0", backend=None):
    """统一命令分发。backend 可选，用于支持按钮交互（/restart、/update 立即提示）。"""
    ...
    try:
        ...
        if cmd == "/restart": return cmd_restart(text, backend)
        if cmd == "/debug": return cmd_debug()
        if cmd == "/update": return cmd_update(backend)
        return "未知命令。\n\n" + cmd_help()
    except Exception as e:
